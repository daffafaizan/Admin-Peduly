...
const AddSlide = ({ match }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    http
      .post(`${API_ENDPOINT.CREATE_SLIDE}`, formData)
      .then((res) => {
  ...
  function validator() {
    if (title && image && startDate && endDate && url) return true
    return false
  }
    ...
    <>
      ...
      <div className="slide-page">
                  ...
                  <FormGroup>
                    <Col className="d-flex justify-content-end mt-5">
                      {validator() ? (
                        ...
                        <Button type="submit" className="px-4" disabled>
                          Buat Slide
                        </Button>
                      )}
                    </Col>
        ...
        <Modal isOpen={isOpen} toggle={() => setIsOpen(false)}>
          <ModalHeader>{modalMsg}</ModalHeader>
          <ModalFooter>
